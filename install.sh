...
docker network create qfofokids --subnet=192.168.32.0/24
docker-compose build app
docker-compose up -d
docker exec -it qfofokids-app php artisan config:cache
...
migrations() {
    sudo docker exec -it qfofokids-app bash -c "php artisan migrate --force"
    menuPrincipal
}

seed() {
    docker exec -it qfofokids-app bash -c "php artisan db:seed --class=StatesSeeder"
